<script lang="ts">
	...
	// 'loading' handles page load, 'submitting' handles the button state
	let loading = true; 
	...
	// Preview: Subscribe to store to show "Recent Entries" at bottom
    ...
    const handleSubmit = async (e: Event) => {
        ...
        // The store now returns string (ID) or null (Error)
...
<div class="min-h-screen text-slate-200 p-4 md:p-8 flex flex-col items-center justify-center font-['Inter'] relative overflow-hidden">
	...
	<div class="w-full max-w-xl bg-[#0E0E10]/80 backdrop-blur-md rounded-2xl shadow-[0_0_40px_-10px_rgba(0,0,0,0.7)] border border-white/10 p-6 md:p-10 relative overflow-hidden group" in:fly={{ y: 20, duration: 500, delay: 100 }}>
		...
		<form on:submit={handleSubmit} class="space-y-8 relative z-10">
			...
			<div class="space-y-2 group/input">
				...
				<input
					...
					class="w-full p-4 rounded-xl bg-black/40 border border-white/10 text-white placeholder-gray-600 focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500 outline-none transition-all font-['Rajdhani'] font-bold text-lg tracking-wide"
				/>
			</div>
			...
			{#if roundRobin}
				<div transition:slide class="rounded-xl border border-cyan-500/20 bg-cyan-900/10 p-6 relative overflow-hidden">
					...
					<p class="text-xs font-bold text-cyan-400 mb-4 uppercase tracking-widest font-mono border-b border-cyan-500/20 pb-2">
						Group Configuration
					</p>
			...
			<button
				disabled={submitting}
				class="w-full py-4 rounded-xl bg-gradient-to-r from-cyan-700 to-cyan-500 hover:from-cyan-600 hover:to-cyan-400 text-white font-black font-['Rajdhani'] text-xl tracking-[0.15em] shadow-lg shadow-cyan-900/20 transition-all hover:scale-[1.02] hover:shadow-cyan-500/40 relative overflow-hidden group disabled:opacity-50 disabled:cursor-not-allowed"
			>
				<span class="relative z-10">{submitting ? 'INITIALIZING...' : 'INITIALIZE'}</span>
