<script lang="ts">
    ...
    const unsubscribe = tournamentStore.subscribe((list) => {
        ...
        // If tournament loaded or updated
        if (found) {
            // Check if we switched tournaments or just updated data
            const isNew = !tournament || tournament.id !== found.id;
            tournament = found;
            
            // If it's a new load or we are syncing, refresh the table
            // We only refresh if currentEntries is empty OR if we just loaded the page
            // to avoid overwriting ongoing edits while typing, 
            // but for simplicity, we'll sync with store on load.
            if (isNew) {
    ...
    function loadMatchData(matchId: number) {
        ...
        // Safety check: tournament.matches might be undefined on old data
    ...
    $: filteredTeams = tournament?.teams.filter((t) => {
        ...
        // Check if team is already in current displayed entries
    ...
    // Edit: Remove from list and put back in inputs
                ...
                {#if uploadedImages.length > 0}
                    <div class="flex gap-2 overflow-x-auto mt-3 pb-2">
                        {#each uploadedImages as img}
                            <img src={img} alt="ref" class="h-24 w-auto rounded-lg border shadow-sm object-cover" />
                        {/each}
                    </div>
                {/if}
            </div>
