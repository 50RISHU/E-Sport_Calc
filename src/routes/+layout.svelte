...
>
	...
	<div
		class={`lg:hidden bg-[#151518] border-t border-white/10 overflow-hidden transition-[max-height] duration-300 ease-in-out ${navOpen ? 'max-h-[500px]' : 'max-h-0'}`}
	>
		<div class="py-4">
			<ul
				class="flex flex-col space-y-3 px-6 font-semibold text-gray-400 uppercase tracking-widest"
			>
				{#if session}
					<li>
						<a
							href="/dashboard"
							class="block py-2 hover:text-cyan-400"
							on:click={() => (navOpen = false)}>Dashboard</a
						>
					</li>
				{/if}
				<li>
					<a href="/" class="block py-2 hover:text-cyan-400" on:click={() => (navOpen = false)}
						>About</a
					>
				</li>
				<li>
