<script lang="ts">
	...
	// 1. Email Login (Standard Protocol)
	...
	// 2. Google OAuth Login (Smart Handling)
	const handleGoogleLogin = async () => {
		loading = true;

		// CRITICAL STEP: Choose the right return address
		// If App: Send them to the Deep Link (com.esportcalc.app://...)
		// If Web: Send them to the standard web URL (https://.../auth/callback)
		...
		const { error } = await supabase.auth.signInWithOAuth({
			...
			options: {
				...
				// "offline" ensures we get a Refresh Token so the user stays logged in
				queryParams: {
					access_type: 'offline',
					prompt: 'consent'
				}
		...
		// Note: loading stays true while redirecting to avoid double-clicks
	};
</script>
		...
		<div class="w-full max-w-sm relative z-10">
			...
			<div class="space-y-6">
				...
				<div class="relative flex py-2 items-center">
					...
					<span class="shrink-0 mx-4 text-gray-600 text-xs uppercase tracking-widest font-mono">Or Standard Protocol</span>
					<div class="grow border-t border-white/10"></div>
				</div>
				...
				<form on:submit={handleLogin} class="space-y-6">
					...
					<div class="flex justify-end">
						<a
							...
							class="text-purple-400 hover:text-purple-300 font-bold hover:underline transition-colors uppercase tracking-wider text-[10px]"
						>
							Lost Security Key?
						</a>
					</div>
			...
			<div class="text-center pt-4 border-t border-white/5 mt-6">
				<p class="text-sm text-gray-500">
					New Operator? <a
						href="/signup"
						class="text-purple-400 hover:text-purple-300 font-bold hover:underline transition-colors uppercase tracking-wider text-xs ml-1"
					>Register Profile</a>
				</p>
			</div>
		</div>
