...
>
	<div class="absolute bottom-5 right-[-15px] text-white/3 group-hover:text-cyan-500/8 pointer-events-none transition-all duration-500 group-hover:scale-110 group-hover:rotate-6 z-0">
	...
	<div class="relative z-10 flex flex-col justify-between h-full p-4 md:p-6">
		<div class="flex justify-between items-start">
			<div class="min-w-0 pr-2">
				<span class="text-[9px] md:text-[10px] text-cyan-600 font-mono font-bold tracking-widest mb-0.5 block">
					ID: {tournament.id.slice(0,4)}
				</span>
				<h3 class="text-xl md:text-2xl font-bold text-white font-['Rajdhani'] leading-none group-hover:text-cyan-50 transition-colors truncate">
					{tournament.name}
				</h3>
			</div>
			<div class="shrink-0 px-1.5 py-0.5 bg-white/5 rounded text-[9px] md:text-[10px] font-bold text-gray-400 border border-white/5 group-hover:border-green-500/30 group-hover:text-green-400 transition-colors shadow-sm">
				ACTIVE
			</div>
		</div>
		...
		<div class="flex gap-4 md:gap-6 my-auto relative">
			<div class="flex flex-col">
				<span class="text-2xl md:text-3xl font-['Rajdhani'] font-bold text-white leading-none">{tournament.teams.length}</span>
				<span class="text-[9px] md:text-[10px] text-gray-500 font-bold tracking-wider uppercase mt-1">Teams</span>
			</div>
			<div class="w-px h-6 md:h-8 bg-white/10 self-center"></div>
			<div class="flex flex-col">
				<span class="text-2xl md:text-3xl font-['Rajdhani'] font-bold text-white leading-none">{tournament.roundRobin ? 'RR' : 'STD'}</span>
				<span class="text-[9px] md:text-[10px] text-gray-500 font-bold tracking-wider uppercase mt-1">Format</span>
			</div>
		</div>
